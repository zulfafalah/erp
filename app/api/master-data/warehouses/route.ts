...
export async function GET(req: NextRequest) {
    try {
        ...
        const raw = await apiFetch<PaginatedResults | DataWrapper | WarehouseListItem[]>(
            `/api/v1/warehouses/?${qs.toString()}`,
            { method: "GET" }
        );

        let data: WarehouseListItem[] = [];
        let total = 0;
...
export async function POST(req: NextRequest) {
    try {
        ...
        const data = await apiFetch<unknown>("/api/v1/warehouses/", {
            method: "POST",
            body,
        });
