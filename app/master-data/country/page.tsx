...
const FILTER_FIELDS: FilterField[] = [
    { key: "cityname", label: "Nama Negara", type: "text" },
];
...
export default function CountryListPage() {
    ...
    const [data, setData]             = useState<CountryListItem[]>([]);
    ...
    const fetchData = useCallback(async (targetPage: number, searchStr: string) => {
        ...
        try {
            ...
            const res  = await fetch(`/api/master-data/countries?${qs.toString()}`);
    ...
    const handlePageChange = (p: number) => {
        setPage(p);
        // fetchData dipanggil via useEffect[page]
    };

    // ── Delete ────────────────────────────────────────────────────────────────

    const handleDelete = async (id: number, name: string) => {
        if (!confirm(`Hapus negara "${name}"? Tindakan ini tidak dapat dibatalkan.`)) return;
        setDeletingId(id);
        try {
            const res  = await fetch(`/api/master-data/countries/${id}`, { method: "DELETE" });
            const json = await res.json() as { ok: boolean; message?: string };
            if (!res.ok || !json.ok) {
                alert(json.message ?? "Gagal menghapus data negara.");
                return;
            }
            fetchData(page, search);
        } catch {
                            ...
                            <div className="block md:hidden divide-y divide-primary/5">
                                {isLoading ? (
                                    ...
                                    <div className="p-12 text-center">
                                        ...
                                        <p className="mt-2 text-sm text-slate-500">Tidak ada data negara</p>
                                    </div>
                                ) : data.map((item) => (
                                    <div key={item.cityid} className="p-4 space-y-3">
                                        <div className="flex justify-between items-start">
                                            <div>
                                                <Link
                                                    href={`/master-data/country/${item.cityid}`}
                                        ...
                                        {isLoading ? (
                                            ...
                                            <tr>
                                                <td colSpan={2} className="px-6 py-16 text-center">
                                                    <span className="material-symbols-outlined text-5xl text-slate-300 block">public</span>
                                                    <p className="mt-2 text-sm text-slate-500">Tidak ada data negara</p>
                                                </td>
                                            </tr>
                                        ) : data.map((item) => (
                                            <tr
                                                ...
                                                onClick={() => router.push(`/master-data/country/${item.cityid}`)}
                                            >
                                                <td className="px-6 py-4">
                                                    <Link
                                                        href={`/master-data/country/${item.cityid}`}
