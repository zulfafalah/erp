...
const apOpeningBalances: APOpeningBalance[] = [
    {
        id: "SAH1812-0003",
        ...
        nilaiAwal: 220570000,
        totalBayar: 0,
        catatan: "Pencatatan Sisa Hutang Pemasok Awal Hutang P. Saham 2017",
    },
    {
        id: "SAH1812-0002",
        ...
        pemasok: "UNILEVER",
        status: "Approved",
        nilaiAwal: 33983266180,
        totalBayar: 33983266180,
        catatan: "Pencatatan Sisa Hutang Pemasok Awal UNILEVER",
    },
    {
        id: "SAH1812-0001",
        ...
        nilaiAwal: 33825000,
        totalBayar: 33825000,
        catatan: "Pencatatan Sisa Hutang Pemasok Awal CV CITRA HARAPAN JAYA",
    },
];

// ─── Status ───────────────────────────────────────────────────────────────────

const statusStyles: Record<APOpeningBalance["status"], string> = {
...
export default function APOpeningBalanceListPage() {
    const [filteredData, setFilteredData] = useState<APOpeningBalance[]>(apOpeningBalances);

    const handleApplyFilter = (rules: FilterRule[]) => {
        if (rules.length === 0) {
            setFilteredData(apOpeningBalances);
            return;
        }
        const result = apOpeningBalances.filter((item) =>
            rules.every((rule) => {
                const { field, operator, value } = rule;
                const itemValue = item[field as keyof APOpeningBalance];
    ...
    const columns: Column<APOpeningBalance>[] = [
        {
            header: "No. Transaksi",
            key: "noTransaksi",
            render: (r) => (
                <Link
                    href={`/finance/accounts-payable/opening-balance/${r.id}`}
    ...
    const renderMobileCard = (r: APOpeningBalance) => (
                        ...
                        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
                            <div>
                                <h2 className="text-2xl md:text-3xl font-black tracking-tight text-slate-900">
                                    Saldo Awal Hutang Pemasok
                                </h2>
                                <p className="text-slate-500 mt-1">
                                    Daftar pencatatan sisa hutang pemasok awal (Opening Balance AP).
                                </p>
                            </div>
                            <div className="flex flex-wrap sm:flex-nowrap items-center gap-2 md:gap-3 w-full md:w-auto mt-2 md:mt-0">
                                <MultiFilter fields={FILTER_FIELDS} onApplyFilter={handleApplyFilter} />
                                <Link
                                    href="/finance/accounts-payable/opening-balance/new"
