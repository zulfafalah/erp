...
import { PurchaseInvoiceListItem } from "./types";
import {
    FILTER_FIELDS,
    buildColumns,
    InvoiceMobileCard,
...
export default function PurchaseInvoiceListPage() {
    const [data,        setData]        = useState<PurchaseInvoiceListItem[]>([]);
    ...
    const fetchData = useCallback(async (
        ...
        try {
            ...
            const paginated = json.data as { count: number; results: PurchaseInvoiceListItem[] };
    ...
    //
    //   select / number field:
    //     key is already the final param name (e.g. statusrcv, rcvdate__gte)
    //     — sent as-is with the rule value.
    ...
    const handleDelete = async (rcvid: number) => {
        setIsDeleting(true);
        try {
            const res = await fetch(`/api/purchase/invoices/${rcvid}`, { method: "DELETE" });
            if (res.ok) {
                setDeletingId(null);
                ...
                <section className="flex-1 flex flex-col bg-background-light overflow-hidden">
                    ...
                    <div className="flex-1 overflow-y-auto p-4 md:p-8 pb-28 md:pb-8 space-y-4 md:space-y-8">
                        ...
                        <DataTable
                            ...
                            emptyText="Tidak ada data. Coba ubah filter atau tambah invoice baru."
                            renderMobileCard={(item) =>
                                InvoiceMobileCard(item, setDeletingId)
                            }
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm px-4">
                    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6">
                        <div className="flex items-center gap-3 mb-4">
                            ...
                            <div>
                                <p className="font-bold text-slate-900">Hapus Nota Pembelian?</p>
