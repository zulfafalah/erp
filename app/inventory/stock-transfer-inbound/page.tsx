...
interface StockTransferInbound {
    id: string;
    noTerima: string;
    noKirim: string;
    gudangAsal: string;
    ...
    alasan: string;
    qty: number;
    status: "Approved" | "Draft" | "In Tran";
}

// ─── Mock Data ────────────────────────────────────────────────────────────────

const transfers: StockTransferInbound[] = [
    {
        id: "SIN2206-0001",
...
const statusStyles: Record<StockTransferInbound["status"], string> = {
    Approved:  "bg-green-100 text-green-800",
    Draft:     "bg-slate-100 text-slate-800",
    "In Tran": "bg-blue-100 text-blue-800",
};
...
export default function StockTransferInboundListPage() {
    const [filteredData, setFilteredData] = useState<StockTransferInbound[]>(transfers);
    ...
    const handleApplyFilter = (rules: FilterRule[]) => {
        ...
        const result = transfers.filter((item) =>
            rules.every((rule) => {
                ...
                const itemValue = item[field as keyof StockTransferInbound];
    ...
    const columns: Column<StockTransferInbound>[] = [
        {
            header: "Status",
            key: "status",
            render: (r) => (
        ...
        },
        {
            ...
            render: (r) => (
                <Link
                    href={`/inventory/stock-transfer-inbound/${r.id}`}
        ...
        {
            header: "Ref Transfer (Kirim)",
            key: "noKirim",
            render: (r) => <span className="text-sm font-medium text-slate-600">{r.noKirim}</span>,
        },
        {
            header: "Alasan",
            key: "alasan",
    ...
    const renderMobileCard = (r: StockTransferInbound) => (
        <div className="p-4 space-y-3">
            <div className="flex justify-between items-start">
                <div>
                    ...
                    <p className="text-xs text-slate-500 mt-0.5">{r.tanggal} · {r.noKirim}</p>
                </div>
                <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${statusStyles[r.status]}`}>
                    {r.status}
                </span>
                        ...
                        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
                            <div>
                                ...
                                <p className="text-slate-500 mt-1">
                                    Kelola dan pantau semua penerimaan perpindahan barang antar gudang (inbound).
                                </p>
                            </div>
                            <div className="flex flex-wrap sm:flex-nowrap items-center gap-2 md:gap-3 w-full md:w-auto mt-2 md:mt-0">
                                <MultiFilter fields={FILTER_FIELDS} onApplyFilter={handleApplyFilter} />
                                <Link
                                    href="/inventory/stock-transfer-inbound/new"
